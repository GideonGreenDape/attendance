...
const document = 'attendance_token';

async function saveAttendaceToken(tokendata) {
    let client;
    try {
...
async function findToken(tokenparameter) {
    let client;
    const now = new Date();
    try {
        client = new MongoClient(url);
...
module.exports = { saveAttendaceToken, findToken };