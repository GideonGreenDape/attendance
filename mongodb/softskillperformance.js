...
async function softskillsPerformance(student_id) {
    let client;
    try {
        // Validate student_id
        if (!student_id) {
...
async function checkTodaysoftskillsAttendance(student_id) {
    let client;
    try {
        client = new MongoClient(url);
        await client.connect();
...
module.exports = {
    softskillsPerformance,
    checkTodaysoftskillsAttendance
};